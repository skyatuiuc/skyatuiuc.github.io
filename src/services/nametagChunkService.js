...
/**
 * Save nametag template parent document and all chunk subdocuments atomically
 ...
 * @returns {Promise<{ chunkCount: number, chunkSize: number, thumbnailBase64: string|null }>}
 */
export async function saveNametagTemplateWithChunks(db, templateMetadata, fullBase64) {
  ...
  const attemptCommit = async (chunkSize) => {
    ...
    let existingDocs = [];
    try {
      const oldSnap = await getDocs(collection(db, 'nametag_templates', templateId, 'chunks'));
    ...
    // Parent Document: nametag_templates/{templateId}
    const parentDocRef = doc(db, 'nametag_templates', templateId);
    const parentPayload = {
      ...templateMetadata,
    ...
    // Ensure raw high-res base64 image strings are never placed in the parent doc
    delete parentPayload.fullBase64;
    ...
    // Chunk Subdocuments: nametag_templates/{templateId}/chunks/{index}
    chunks.forEach((chunk) => {
      const chunkDocRef = doc(db, 'nametag_templates', templateId, 'chunks', String(chunk.index));
  ...
  try {
    ...
    console.warn("Primary nametag chunk batch notice, retrying with 500KB fallback:", primaryErr);
 ...
 * @returns {Promise<string|null>}
 */
export async function loadNametagTemplateImage(db, templateId) {
  ...
  try {
    const parentDocRef = doc(db, 'nametag_templates', templateId);
    const chunksCollRef = collection(db, 'nametag_templates', templateId, 'chunks');
    ...
    console.warn("Failed to fetch nametag template image chunks:", err);
    return null;
  }
}
 ...
 * @param {string} templateId Template Document ID
 */
export async function deleteNametagTemplateWithChunks(db, templateId) {
  ...
  try {
    ...
    batch.delete(doc(db, 'nametag_templates', templateId));

    await batch.commit();
    imageMemoryCache.delete(templateId);
  } catch (err) {
    console.warn("Error deleting nametag template chunks:", err);
    throw err;
  }
}

/**
 * Fetch all global nametag templates from Firestore
 * @param {any} db Firestore instance
 * @returns {Promise<Array<object>>}
 */
export async function getNametagTemplates(db) {
  if (!db) return [];
  try {
    const tplColl = collection(db, 'nametag_templates');
    let snapshot;
    try {
      snapshot = await getDocs(query(tplColl, orderBy('createdAt', 'desc')));
    } catch {
      // Fallback if index not yet ready or ordered query fails
      snapshot = await getDocs(tplColl);
    }
    ...
    console.warn("Failed to fetch nametag templates:", err);
    return [];
  }
}
